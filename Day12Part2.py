...
def numArrangments(springs, groups):
    ...
    if len(groups) == 0 and len(springs) > 0:
        ...
        return 0
    if springs[0] == ".":
        result = numArrangments(springs[1:], groups)
    ...
    if springs[0] == "#":
        ...
        if pointer == int(groups[0]):
            ...
            result = numArrangments(springs[(pointer+1):], groups[1:])
            knownArrangements[springs[(pointer+1):] + str(groups[1:])] = result
            return result
        if pointer < len(springs) and springs[pointer] == "?":
            result1 = numArrangments(springs[:pointer] + "." + springs[pointer+1:], groups)
            result2 = numArrangments(springs[:pointer] + "#" + springs[pointer+1:], groups)
        ...
        return 0
    result1 = numArrangments("." + springs[1:], groups)
    result2 = numArrangments("#" + springs[1:], groups)
